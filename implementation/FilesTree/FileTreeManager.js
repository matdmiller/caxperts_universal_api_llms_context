...
exports.resolveFilesTreeElement = resolveFilesTreeElement;
...
class FileTreeManager {
    ...
    async getChildren(folder) {
        ...
        apiResult.ResultData.FilesTreeObjects.forEach((element) => {
            result.push(resolveFilesTreeElement(element));
        });
        return result;
    }
    /**
    ...
    async importContainer(parameter) {
        ...
        return resolveFilesTreeElement(response.ResultData.FilesTreeObject);
    }
    /**
    ...
    async createSketch(parent, name) {
        ...
        const result = (await APIConnector_1.Api.get().sendCommandWithReturnType(command)).ResultData
            .FilesTreeObject;
        return resolveFilesTreeElement(result);
    }
    /**
     ...
     */
    async createAnimation(sketch, name) {
        const command = new CaxApiCommand_1.CaxApiCommand(Util_1.ApiCommands.FilesTreeCreateAnimation);
        command.commandParameters.push(name);
        command.commandParameters.push(sketch.Id.toString());
        const result = (await APIConnector_1.Api.get().sendCommandWithReturnType(command)).ResultData
            .FilesTreeObject;
        return resolveFilesTreeElement(result);
    }
 ...
 */
function resolveFilesTreeElement(element) {
    if (element == null)
        return null;
    switch (element.Type) {
        ...
        default:
            Logger_1.logger.debug("Could not resolve " + element.Type);
